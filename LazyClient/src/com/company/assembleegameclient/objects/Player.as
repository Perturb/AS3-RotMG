...
public class Player extends Character
    {
        ...
        public var chpMax:int = -1;
        public var chpBoost:int = -1;
        private var vitTime:int = -1;
        public var remBuffTime:Vector.<int> = new Vector.<int>();
        public var cMaxSeal:int = -1;
        public var cItems:Array = [-1, -1, -1, -1];
        public var cItemshp:Array = [0, 0, 0, 0];
        private var unconfuseDelay:int = -1;
        private var wasConfused:Boolean = false;
        //public var averageSyncTime:Number = 100;
        //private var totalSyncs:int = 1;
        //private var serverHP:Vector.<int> = new Vector.<int>();
        //private var serverHPTime:Vector.<int> = new Vector.<int>();
        ...
        private function checkConfuse():Boolean
        {
            if (!Parameters.data_.noConfuse)
            {
                return isConfused();
            }
            if (Parameters.data_.confuseRecoveryMinMS == -1)
            {
                return false;
            }

            if (isConfused() && (unconfuseDelay == -1))
            {
                unconfuseDelay = getTimer() + (Math.round(Math.random() + 5)) + Parameters.data_.confuseRecoveryMinMS;
                wasConfused = true;
            }
            ...
            if (getTimer() >= unconfuseDelay)
            {
                if (wasConfused == false)
                {
                    unconfuseDelay = -1;
                }
                return false;
            }
            else if (unconfuseDelay != -1)
            {
                return true;
            }
            return false;
        ...
        {
            if ((!map_.gs_.isSafeMap) && ((((chp / chpMax) * 100) <= Parameters.data_.autoNexus) && (!(Parameters.data_.autoNexus == 0))))
            {
        ...
        public function handleItemHp():int
        {
            var _local_1:XML;
            var _local_2:XML;
            var _local_3:int;
                ...
                if (equipment_[_local_4] != -1)
                {
                    ...
                    if (_local_1.hasOwnProperty("ActivateOnEquip"))
                    {
                        for each (_local_2 in _local_1.ActivateOnEquip)
                        {
                            if (_local_2.@stat == 0)
                            {
                                _local_3 = (_local_3 + int(_local_2.@amount));
                                if (equipment_[_local_4] != cItems[_local_4])
                                {
                                    cItems[_local_4] = equipment_[_local_4];
                                    chp = ((chp < (chpMax - cItemshp[_local_4]) ? (chp) : (chpMax - cItemshp[_local_4])));
                                    cItemshp[_local_4] = int(_local_2.@amount);
                                }
                            }
                        }
                    }
        ...
        {
            var _local_2:CharacterStatusText;
            var _local_3:Number;
            var _local_4:Number;
            var _local_5:Number;
            ...
            if (this == map_.player_)
            {
                ...
                if (Parameters.data_.autoNexusDebug) {
                    if ((hp_ > (chp + 3)) || ((hp_ < (chp - 3)) && chp < chpMax)) {
                        serverHP.push(hp_);
                        serverHPTime.push(_local_10);
                    }
                    _local_8 = 0;
                    while (_local_8 < serverHP.length)
                    {
                        if ((_local_10 - serverHPTime[_local_8]) > 300)
                        {
                            serverHP.splice(_local_8, 1);
                            serverHPTime.splice(_local_8, 1);
                            continue;
                        }
                        if ((serverHP[_local_8] < (chp + 3)) && (serverHP[_local_8] > (chp - 3))) {
                            if ((_local_10 - serverHPTime[_local_8]) > (averageSyncTime * 1.5))
                            {
                                this.addTextLine.dispatch(ChatMessage.make("", ("Sync took " + (_local_10 - serverHPTime[_local_8]) + " ms."), -2));
                                this.addTextLine.dispatch(ChatMessage.make("", ("Average sync time: " + averageSyncTime + " ms."), -2));
                            }
                            averageSyncTime = (((averageSyncTime * totalSyncs) + (_local_10 - serverHPTime[_local_8])) / (totalSyncs + 1));
                            serverHP.splice(_local_8, 1);
                            serverHPTime.splice(_local_8, 1);
                            totalSyncs++;
                            continue;
                ...
                */
                if (((!(vitTime == -1)) && (!(isPaused()))))
                {
                    if (isBleeding())
                    {
                        chp = (chp - ((_local_10 - vitTime) * 0.02));
                    }
                    else
                    {
                        if (!isSick())
                        {
                            if (isHealing())
                            {
                                chp = (chp + ((_local_10 - vitTime) * Number(((21 + (0.12 * vitality_)) / 1000))));
                            }
                            else
                            {
                                chp = (chp + ((_local_10 - vitTime) * Number(((1 + (0.12 * vitality_)) / 1000))));
                            }
                        }
                    }
                    if (this.breath_ == 0)
                    {
                        chp = (chp - ((_local_10 - vitTime) * 0.094));
                    }
                    this.checkOPAuto();
                }
                vitTime = _local_10;
                ...
                _local_8 = (this.handleItemHp() + _local_8);
                if (chpBoost != _local_8)
                {
                    chpMax = ((chpMax - chpBoost) + _local_8);
                    chpBoost = _local_8;
                }
                if (chp > chpMax)
                {
        ...
        {
            if ((Parameters.data_.lockFilter) && ((this != map_.player_) && (!this.starred_)))
            {
                return;
            }
