...
package com.company.assembleegameclient.objects
{
    ...
    public class Player extends Character 
    {
        ...
        public var remBuffTime:Vector.<int> = new Vector.<int>(0);
        public var cmaxseal:int;
        //public var remBuffValue:Vector.<int> = new Vector.<int>(0);
        //public var topBuffIndex:int;
        ...
        public function setRelativeMovement(_arg_1:Number, _arg_2:Number, _arg_3:Number):void
        {
            ...
            if ((isConfused()) && (!Parameters.data_.noConfuse))
            {
        ...
        override public function update(_arg_1:int, _arg_2:int):Boolean
        {
            ...
            if (this == map_.player_)
            {
                if (((!(vitTime == -1)) && (!(isPaused()))))
                {
                    ...
                    else
                    {
                        if (!isSick())
                        {
                            if (isHealing())
                            {
                                chp = (chp + ((getTimer() - vitTime) * Number(((21 + (0.12 * vitality_)) / 1000))));
                            }
                            else
                            {
                                chp = (chp + ((getTimer() - vitTime) * Number(((1 + (0.12 * vitality_)) / 1000))));
                            };
                        };
                    };
                    if (this.breath_ == 0)
                    {
                        chp = (chp - ((getTimer() - vitTime) * 0.094));
                    };
                    this.checkOPAuto();
                };
                vitTime = getTimer();
                ...
                if (!(cmaxhpboost == _local_8))
                {
                    cmaxhp = ((cmaxhp - cmaxhpboost) + _local_8);
                    cmaxhpboost = _local_8;
                };
                if (chp > cmaxhp)
                {
                    chp = cmaxhp;
